...
def uploadedPic(request):
    if request.method == 'POST':
        ...
        global filedir
        filedir = os.path.join(BASE_DIR, 'media', 'uk.txt')
        ...
        if len(myList) < 1:
            messages.error(request, 'Sorry, No Face Has Detected. Please Choose an Image with Two Cleared Eyes.')
            return render(request, 'base/home.html', {})
        ...
        perc = '%'
        context={'winner':winner,'messi':messi,'maria':maria,'roger':roger,
                 'serena':serena,'virat':virat,'image':imgdir, 'perc':perc}
